...
/*							-can be numeric or character variable type*/
/*		newsample = (OPTIONAL) creates new boostrap sample dataset*/
/*							-must be numeric value*/
/*							1: create new dataset*/
/*							0 and all other numeric values: use existing boostrap dataset */
/*					DEFAULT: 1 (create new dataset) */
...
%macro bootauc(dsn=,marker=,outcome=,id=,newsample=1,boot=100,view=1);
%if &newsample=1 %then %do; 
	...
	run;
%end;
%else %if %sysfunc(exist(work.bootsample)) = 1 %then %do;
	...
	%end;
%end;
%else %put "ERROR: Bootsample dataset has not been created";
